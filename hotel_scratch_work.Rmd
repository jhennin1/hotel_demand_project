...
hotel_demand_factor <- hotel_demand %>%
  mutate(
    is_canceled = as.factor(is_canceled),
    is_repeated_guest = as.factor(is_repeated_guest)
    )

hotel_demand_numeric <- hotel_demand %>%
  select(
...
set.seed(253)
hotel_split <- initial_split(hotel_demand_factor, prop = .7, 
                             strata = is_canceled)
...
hotel_train %>% 
  ggplot(aes(x = is_canceled, fill = hotel)) +
  geom_bar(position = "fill") +
