...
```{r, fig.width=20, fig.cap="Plot detailing the correlation between individual numeric variables included in the data set. The opacity of the square signifies the magnitude, with blue representing positive correlation and red representing negative correlation."}
...
We wanted to create the "best" possible model that would be fit for predicting whether or not a particular reservation would be canceled or not. When we say "best" possible model, we want to have a model that has the highest testing accuracy since the we can measure a model's predictive power based on its performance on the testing data set.

In our study, we created five models with is_canceled as our response variable. These five models (logistic, LASSO, Classifcation Tree, and Random Forest, and a logistic model with seven variables) and their outputs can be seen below. We trained each model on the training set and calculated the training and cv accuracies as well as the areas under each ROC curve. The code chunks below give a run down of all of these metrics which assisted us in our decision to isolate one model as the best one. 
...
Our second model is the full logistic regression model with all of our predictors. As seen in our results table down below, this model was one of our poorest performing models in terms of the testing accuracy and the testing AUC. 
...
Our third model was a LASSO model where we used all of the predictors in our training set. It had similar results to our full logistic regression model. As seen in the lambda vs. accuracy graph...
...
As aforementioned, the predictor, lead_time, is the most important variable in our random forest model per the variable importance plot above. The partial dependence plot tells us the effect that lead_time has on the probability of a reservation being canceled. For small values of lead time, the y_hat value is close to 1 signaling that these quickly-made reservations will most likely be canceled. The probability of cancelation quickly tapers off as lead time increases which means that reservations made far in advance will most liekly be kept, holding all other predictors constant.
...
After completing training and testing for our five models and analysing their summaries, confusion matricies, and ROC plots, we came to the conclusion that our random forest model, hotel_ranf_oob, is our best model as it not only performed the best on the training data set but also performed exceptionally well with the testing data set. In terms of interpretaion, this tree is complex given the number of variables that we implementing into the model; however, combining the natural intuition of trees along with the information that variable importance plots and partial dependence plots give us, we can get a pretty good understanding of our hotel booking situation.

In terms of future work, the idea of clustering caught our eye as a possible next step to further hone our model's predictive performance. The hotel_bookings data set is quite large with over 100,000 observations. Using non-supervised machine learning techniques, like hierarchical clustering, would allow us to narrow this data set into like groups which could allows us to make models that focus on certain elements of a hotel reservation.
